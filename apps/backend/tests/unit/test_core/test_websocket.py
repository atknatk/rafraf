...
class TestConnectionManager:
    ...
    async def test_connect_returns_unique_ids(self, manager: ConnectionManager) -> None:
    ...
    async def test_disconnect_removes_connection(
        ...
        cid = await manager.connect(websocket=mock_websocket, user_id="user-1", session_id="sess-1")
        assert manager.active_count == 1
        await manager.disconnect(cid)
        assert manager.active_count == 0

    async def test_disconnect_nonexistent_id(self, manager: ConnectionManager) -> None:
    ...
    async def test_send_json_to_unknown_id(self, manager: ConnectionManager) -> None:
    ...
    async def test_broadcast_json(self, manager: ConnectionManager) -> None:
    ...
    async def test_get_connection_unknown(self, manager: ConnectionManager) -> None:
        """get_connection() should return None for unknown ID."""
        assert manager.get_connection("unknown") is None

    async def test_get_user_connections(self, manager: ConnectionManager) -> None:
    ...
    async def test_get_user_connections_empty(self, manager: ConnectionManager) -> None:
